		...
		if (arg0.getSource() == jb1) {
			...
			if (!cm.updateItem(params)) {
				...
				if (params[8].equals("无")
						| tuangou_lable.getText().equals("暂无")) {
					deleteFile();
				}
			}
		}
			...
			if (tg.equals("无") & !tuangouhao.equals("无")) {
				...
				for (int x = 0; x < ml.length; x++) {
					if (ml[x]
							.getClass()
							.getName()
							.equals("com.partys.listener.MyJButtonMouseMoveListener")) {
						break;
					}
					if (x == ml.length - 1) {
						addListener(add);
	...
	public void mouseClicked(MouseEvent e) {
		...
		if (e.getSource() == delete) {
			...
			for (int x = 0; x < ml.length; x++) {
				...
				if (ml[x].getClass().getName()
						.equals("javax.swing.plaf.basic.BasicButtonListener")) {
					delete.removeMouseListener(this);
				}
