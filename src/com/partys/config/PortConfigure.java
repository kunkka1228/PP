...
public class PortConfigure extends CommonJPanel implements ActionListener{
	...
	private JLabel title,content,icon,username_label,port_label,password,status,url,database_label;
	private JTextField username_textfiled,port_textfield,url_textfiled,database_textfiled;
	...
	private void init() {
		...
		status=new JLabel();
		status.setBounds(170, 230, 80, 65);
		test=new JButton("测试");	
	...
	public void actionPerformed(ActionEvent e) {
		// TODO 自动生成的方法存根
		if(e.getSource()==test){
			String url="jdbc:mysql://"+url_textfiled.getText()+":"+port_textfield.getText()+"/"+database_textfiled.getText()+"?characterEncoding=gbk&useSSL=true";
			String username=username_textfiled.getText();
			String pass=new String(pwd.getPassword());
			if(conModel.testDataBase(url, username, pass)){
				status.setIcon(new ImageIcon("image/configure/right.png"));
			}
			else{
				status.setIcon(new ImageIcon("image/configure/wrong.png"));
			}
		}
